...
export const COOK_CSS = `
  ...
  * { box-sizing: border-box; }
  ...
  @media (max-width: 760px) {
    ...
    /* Phase 23 (v2.1 Mobile Kitchen) — one-handed at the stove (MCOOK-01): pin the
       step nav (‹ Back / Next ›) to the viewport bottom while cooking a long step,
       so it's always in the thumb zone. Scoped to body.mode-wizard so it only
       applies in Step-by-step mode; ≤760px only, so the wide cook rendering and
       @media print (which hides .wizard) are untouched. Paper ground matches the
       body so step text never bleeds through. */
    body.mode-wizard .wizard-nav {
      position: sticky;
      bottom: 0;
      margin-top: 16px;
      padding: 10px 0 calc(10px + env(safe-area-inset-bottom));
      background: #F3F2EC;              /* paper (enamel) — matches body */
      border-top: 1px solid #E3E1D8;    /* hairline */
      z-index: 5;
    }
  }
